            ...
            <div class="card-body">
                ...
                <form action="{{ route('coupons.update', $coupon->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" id="value" name="value" value="{{ old('value', $coupon->value) }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="min_value" class="form-label">Giá Trị Tối Thiểu (VNĐ)</label>
                        <input type="number" class="form-control" id="min_value" name="min_value" value="{{ old('min_value', $coupon->min_value) }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="max_value" class="form-label">Giá Trị Tối Đa (VNĐ)</label>
                        <input type="number" class="form-control" id="max_value" name="max_value" value="{{ old('max_value', $coupon->max_value) }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="condition" class="form-label">Điều Kiện</label>
                        <input type="text" class="form-control" id="condition" name="condition" value="{{ old('condition', $coupon->condition) }}" required>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-control" id="status" name="status" required>
                            <option value="Hoạt động" {{ old('status', $coupon->status) == 'Hoạt động' ? 'selected' : '' }}>Hoạt động</option>
                            <option value="Ngừng hoạt động" {{ old('status', $coupon->status) == 'Ngừng hoạt động' ? 'selected' : '' }}>Ngừng hoạt động</option>
                        </select>
                    </div>
